...
class DexFile(object):
    ...
    def write_header(self):
        wf=open(self.filepath+".parsed\\header.hdr",'a')
        wf.writelines('[+] magic:\t0x' + self.DexHeader.magic+"\r")
        wf.writelines('[+] checksum:\t0x' + self.DexHeader.checksum+"\r")
        wf.writelines('[+] signature:\t' + self.DexHeader.signature+"\r")
        wf.writelines('[+] file_size:\t0x' + self.DexHeader.file_size+"\r")
        wf.writelines('[+] header_size:\t0x' + self.DexHeader.header_size+"\r")
        wf.writelines('[+] endian_tag:\t0x' + self.DexHeader.endian_tag+"\r")
        wf.writelines('[+] link_size:\t0x' + self.DexHeader.link_size+"\r")
        wf.writelines('[+] link_off:\t0x' + self.DexHeader.link_off+"\r")
        wf.writelines('[+] map_off:\t0x' + self.DexHeader.map_off+"\r")
        wf.writelines('[+] string_ids_size:\t0x' + self.DexHeader.string_ids_size+"\r")
        wf.writelines('[+] string_ids_off:\t0x' + self.DexHeader.string_ids_off+"\r")
        wf.writelines('[+] type_ids_size:\t0x' + self.DexHeader.type_ids_size+"\r")
        wf.writelines('[+] type_ids_off:\t0x' + self.DexHeader.type_ids_off+"\r")
        wf.writelines('[+] proto_ids_size:\t0x' + self.DexHeader.proto_ids_size+"\r")
        wf.writelines('[+] proto_ids_off:\t0x' + self.DexHeader.proto_ids_off+"\r")
        wf.writelines('[+] field_ids_size:\t0x' + self.DexHeader.field_ids_size+"\r")
        wf.writelines('[+] field_ids_off:\t0x' + self.DexHeader.field_ids_off+"\r")
        wf.writelines('[+] method_ids_size:\t0x' + self.DexHeader.method_ids_size+"\r")
        wf.writelines('[+] method_ids_off:\t0x' + self.DexHeader.method_ids_off+"\r")
        wf.writelines('[+] class_defs_size:\t0x' + self.DexHeader.class_defs_size+"\r")
        wf.writelines('[+] class_defs_off:\t0x' + self.DexHeader.class_defs_off+"\r")
        wf.writelines('[+] data_size:\t0x' + self.DexHeader.data_size+"\r")
        wf.writelines('[+] data_off:\t0x' + self.DexHeader.data_off+"\r")
    ...
    def write_stringid(self):
        wf=open(self.filepath+".parsed\\string.txt",'a')
        for index in range(len(self.DexStringIdList)):
            #wf.writelines("%s %s \r"%(hex(index),self.DexStringIdList[index]))
            wf.writelines("%s \r"%(self.DexStringIdList[index]))
        wf.close()
    ...
    def write_DexTypeId(self):
        wf=open(self.filepath+".parsed\\proto.txt",'a')
        for index in range(len(self.DexTypeIdList)):
            wf.writelines('%s\r' % (self.getDexTypeId(index)))
    ...
    def write_field(self):
        wf=open(self.filepath+".parsed\\field.txt",'a')
        for index in range(len(self.DexFieldIdList)):
            self.DexHeader.f.seek(self.DexFieldIdList[index].offset, 0)
            classIdx = self.DexFieldIdList[index].classIdx
            typeIdx = self.DexFieldIdList[index].typeIdx
            nameIdx = self.DexFieldIdList[index].nameIdx
            wf.writelines('#%s (%s~%s)\r' % (hex(index), hex(self.DexFieldIdList[index].offset), hex(self.DexFieldIdList[index].offset + self.DexFieldIdList[index].length)))
            wf.writelines('DexFieldId[%d]->classIdx=%s\t#%s\r' % (index, hex(classIdx), self.getDexStringId(classIdx)))
            wf.writelines('DexFieldId[%d]->typeIdx=%s\t#%s\r' % (index, hex(typeIdx), self.getDexStringId(typeIdx)))
            wf.writelines('DexFieldId[%d]->nameIdx=%s\t#%s\r' % (index, hex(nameIdx), self.getDexStringId(nameIdx)))
            wf.writelines('\r')
        wf.close()
    ...
    def write_DexMethodId(self):
        wf=open(self.filepath+".parsed\\method.txt",'a')
        for index in range(len(self.DexMethodIdList)):
            ...
            #wf.writelines('#%s (%s~%s)\r' % (hex(index), hex(self.DexMethodIdList[index].offset), hex(self.DexMethodIdList[index].offset + self.DexMethodIdList[index].length)))
            #wf.writelines('DexMethodId[%d]->classIdx=%s\t#%s\r' % (index, hex(classIdx), self.getDexTypeId(classIdx)))
            #wf.writelines('DexMethodId[%d]->protoIdx=%s\t#%s\r' % (index, hex(protoIdx), self.DexProtoIdList[protoIdx].toString(self)))
            #wf.writelines('DexMethodId[%d]->nameIdx =%s\t#%s\r' % (index, hex(nameIdx), self.DexStringIdList[nameIdx]))
            method_kz=self.getDexTypeId(classIdx).replace(';','')
            wf.writelines(method_kz+'.'+self.DexStringIdList[nameIdx])
            wf.writelines('\r')
        wf.close()
...
#            print '\n'
    ...
    def write_DexClassDef(self):
        wf=open(self.filepath+".parsed\\classdef.txt",'a')
        for index in range(len(self.DexClassDefList)):
            dexClassDefObj = self.DexClassDefList[index]
            #wf.writelines( '#%s~%s\r' % (hex(dexClassDefObj.offset), hex(dexClassDefObj.offset + dexClassDefObj.length)))
            #wf.writelines( 'DexClassDef[%d]:\t\r' % index)
            class_kz=self.getDexTypeId(dexClassDefObj.classIdx).replace(';','')
            wf.writelines( '%s\r' % (class_kz))
            #wf.writelines( 'DexClassDef[%d]->classIdx\t= %s\t#%s\r' % (index, hex(dexClassDefObj.classIdx), self.getDexTypeId(dexClassDefObj.classIdx)))
            #wf.writelines( 'DexClassDef[%d]->accessFlags\t= %s\r' % (index, hex(dexClassDefObj.accessFlags) ))
            #wf.writelines( 'DexClassDef[%d]->superclassIdx\t= %s\t#%s\r' % (index, hex(dexClassDefObj.superclassIdx), self.getDexTypeId(dexClassDefObj.superclassIdx)))
            #wf.writelines( 'DexClassDef[%d]->interfaceOff\t= %s\r' % (index, hex(dexClassDefObj.interfaceOff)))
            #if dexClassDefObj.sourceFieldIdx == 0xffffffff:
            #    wf.writelines( 'DexClassDef[%d]->sourceFieldIdx\t= %s\t#UNKNOWN\r' % (index, hex(dexClassDefObj.sourceFieldIdx)))
            #else:
            #    wf.writelines( 'DexClassDef[%d]->sourceFieldIdx\t= %s\t#%s\r' % (index, hex(dexClassDefObj.sourceFieldIdx), self.DexStringIdList[dexClassDefObj.sourceFieldIdx]))
            #wf.writelines( 'DexClassDef[%d]->annotationsOff\t= %s\r' % (index, hex(dexClassDefObj.annotationsOff)))
            #wf.writelines( 'DexClassDef[%d]->classDataOff\t= %s\r' % (index, hex(dexClassDefObj.classDataOff)))
            #wf.writelines( 'DexClassDef[%d]->staticValueOff\t= %s\r' % (index, hex(dexClassDefObj.staticValueOff)))
            #if dexClassDefObj.classDataOff == 0:
            #    continue
            #wf.writelines( '------------------------------------------------------------------------\r')
            #wf.writelines( '# %s~%s\r' % (hex(dexClassDefObj.header.offset), hex(dexClassDefObj.header.offset + dexClassDefObj.header.length)))
            #wf.writelines( 'DexClassDef[%d]->DexClassData->DexClassDataHeader->staticFieldsSize \t= %s\r' % (index, hex(dexClassDefObj.header.staticFieldsSize)))
            #wf.writelines( 'DexClassDef[%d]->DexClassData->DexClassDataHeader->instanceFieldsSize \t= %s\r' % (index, hex(dexClassDefObj.header.instanceFieldsSize)))
            #wf.writelines( 'DexClassDef[%d]->DexClassData->DexClassDataHeader->directMethodsSize \t= %s\r' % (index, hex(dexClassDefObj.header.directMethodsSize)))
            #wf.writelines( 'DexClassDef[%d]->DexClassData->DexClassDataHeader->virtualMethodsSize \t= %s\r' % (index, hex(dexClassDefObj.header.virtualMethodsSize)))
            #if len(dexClassDefObj.staticFields) > 0:
            #    wf.writelines( '------------------------------------------------------------------------\r')
            #    wf.writelines( '# %s~%s\r' % (hex(dexClassDefObj.staticFields[0].offset), hex(dexClassDefObj.staticFields[-1].offset + dexClassDefObj.staticFields[-1].length)))
            #if len(dexClassDefObj.staticFields) < 0 and len(dexClassDefObj.instanceFields) > 0:
            #    wf.writelines( '------------------------------------------------------------------------\r')
            #    wf.writelines( '# %s~%s\r' % (hex(dexClassDefObj.instanceFields[0].offset), hex(
            #        dexClassDefObj.instanceFields[-1].offset + dexClassDefObj.instanceFields[-1].length)))
            #lastFieldIdx = 0
            #for k in range(len(dexClassDefObj.staticFields)):
            #    currFieldIdx = lastFieldIdx + dexClassDefObj.staticFields[k].fieldIdx
            #    fieldName = self.getDexStringId(self.DexFieldIdList[currFieldIdx].nameIdx)
            #    lastFieldIdx = currFieldIdx
            #    wf.writelines( 'DexClassDef[%d]->DexClassData->staticFields[%d]\t= %s\t#%s\r' % (index, k, fieldName, dexClassDefObj.staticFields[k]))
#
            #lastFieldIdx = 0
            #for k in range(len(dexClassDefObj.instanceFields)):
            #    currFieldIdx = lastFieldIdx + dexClassDefObj.instanceFields[k].fieldIdx
            #    fieldName = self.getDexStringId(self.DexFieldIdList[currFieldIdx].nameIdx)
            #    lastFieldIdx = currFieldIdx
            #    wf.writelines( 'DexClassDef[%d]->DexClassData->instanceFields[%d]\t= %s\t#%s\r' % (index, k, fieldName, dexClassDefObj.instanceFields[k]))
#
            #if len(dexClassDefObj.staticFields) + len(dexClassDefObj.instanceFields) > 0:
            #    wf.writelines( '------------------------------------------------------------------------\r')
#
            ...
            #    dexMethodIdObj = self.DexMethodIdList[currMethodIdx]
            #    lastMethodIdx = currMethodIdx
            #    wf.writelines( '# %s~%s\r' % (hex(dexClassDefObj.directMethods[k].offset), hex(dexClassDefObj.directMethods[k].offset + dexClassDefObj.directMethods[k].length)))
            #    wf.writelines( 'DexClassDef[%d]->DexClassData->directMethods[%d]\t= %s\t#%s\r' % (index, k, dexMethodIdObj.toString(self), dexClassDefObj.directMethods[k]))
            #    self.dumpDexCode(dexClassDefObj.directMethods[k],wf)
            #    wf.writelines( '------------------------------------------------------------------------\r')
#
            #lastMethodIdx = 0
            #for k in range(len(dexClassDefObj.virtualMethods)):
            ...
            #    dexMethodIdObj = self.DexMethodIdList[currMethodIdx]
            #    lastMethodIdx = currMethodIdx
            #    wf.writelines( '# %s~%s\r' % (hex(dexClassDefObj.virtualMethods[k].offset), hex(dexClassDefObj.virtualMethods[k].offset + dexClassDefObj.virtualMethods[k].length)))
            #    wf.writelines( 'DexClassDef[%d]->DexClassData->virtualMethods[%d]\t= %s\t#%s\r' % (index, k, dexMethodIdObj.toString(self), dexClassDefObj.virtualMethods[k]))
            #    self.dumpDexCode(dexClassDefObj.virtualMethods[k],wf)
            #    wf.writelines( '------------------------------------------------------------------------\r')
            #wf.writelines('\n')
        wf.close()
...
#            if smaliCode == 'nop':
#                break
    def dumpDexCode(self, dexMethod,writefile):
        if dexMethod.dexCode == None:
            return
        writefile.writelines('# %s~%s\r' % (hex(dexMethod.dexCode.offset), hex(dexMethod.dexCode.offset + dexMethod.dexCode.length)))
        writefile.writelines('DexCode=%s\r' % dexMethod.dexCode)
        offset = 0
        insnsSize = dexMethod.dexCode.insnsSize * 4
        ...
        while offset < insnsSize:
            ...
            insns = dexMethod.dexCode.insns[decodedInstruction.offset:decodedInstruction.offset + decodedInstruction.length]
            writefile.writelines('\t%-16s|%04x: %s\r' % (insns, offset/4, smaliCode))
            offset += len(insns)
...
def writeDex(file):
    ...
    dex.write_stringid()
    dex.write_field()
    dex.write_DexMethodId()
    dex.write_DexClassDef()
    dex.write_DexTypeId()

if __name__=="__main__":
    #sys.argv[1]表示文件的绝对路径
    os.mkdir(sys.argv[1]+'.parsed')
    writeDex(sys.argv[1])